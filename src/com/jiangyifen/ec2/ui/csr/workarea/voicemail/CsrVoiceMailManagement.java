package com.jiangyifen.ec2.ui.csr.workarea.voicemail;
...
public class CsrVoiceMailManagement extends VerticalLayout implements Button.ClickListener{
...
//		gridLayout.addComponent(buildOutlineComboBox(allGovernedDeptIds), 3, 1);
		...
		// 呼叫结果选择框
		gridLayout.addComponent(new Label("电话号码："), 0, 1);
		phoneNumberField=new TextField();
		gridLayout.addComponent(phoneNumberField, 1, 1);
...
//		}
		...
		/**
		 * inner join 也相当于同时加载两张表的数据查询，耗费性能也比较大
		 */
		sqlSelect="select v.id,v.userid,u.username,v.cusnum,v.fromOutline,v.origtimeDatetime,v.duration,v.partfilename from ec2_voicemail v left join ec2_user u on v.userid=u.id where v.domainid="+loginUser.getDomain().getId()+" and u.id = " + loginUser.getId();
		sqlCount="select count(v.id) from ec2_voicemail v left join ec2_user u on v.userid=u.id where v.domainid="+loginUser.getDomain().getId()+" and u.id = " + loginUser.getId();
