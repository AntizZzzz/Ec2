...
public class OutgoingDialForHistoryServiceWindow extends Window implements CloseListener {
	...
	private HistoryServiceRecordCreator serviceOutgoingRecordCreator;			// 外呼结果的创建界面
	...
	public OutgoingDialForHistoryServiceWindow() {
...
//		customerBaseInfoView.setCustomerInfoPanelHeight("165px");
		...
		// 创建客服记录
		serviceOutgoingRecordCreator = new HistoryServiceRecordCreator(this);
		serviceOutgoingRecordCreator.setMargin(true);
		businessOperations.addTab(serviceOutgoingRecordCreator, "创建服务记录", ResourceDataCsr.create_record_16_ico);
	...
	public void echoInformations(CustomerResource customerResource) {
		...
		// 第二步：回显个组件中的信息
		serviceOutgoingRecordCreator.setCustomerResource(customerResource);			// 回显创建记录组件中客户的级别
	...
	public void windowClose(CloseEvent e) {
		serviceOutgoingRecordCreator.clearComponentsValue();
		if(commodityOrderCreator != null) {
	...
	/**
	 * 为我的客服记录界面使用
	 * @param serviceRecordTableFlip 客服记录存放表格的翻页组件
	 */
	public void setServiceRecordTableFlip(FlipOverTableComponent<CustomerServiceRecord> serviceRecordTableFlip) {
		serviceOutgoingRecordCreator.setServiceRecordTableFlip(serviceRecordTableFlip);
	}
