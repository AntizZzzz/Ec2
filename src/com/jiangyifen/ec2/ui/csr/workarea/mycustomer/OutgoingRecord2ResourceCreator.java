...
public class OutgoingRecord2ResourceCreator extends VerticalLayout implements ClickListener, ValueChangeListener {
	// 日志工具
	...
	private FlipOverTableComponent<CustomerResource> prprietaryCustomersTableFlip;		// 服务记录表格的翻页组件
	...
	private CustomerResource customerResource;	// 用户获取服务记录中当前被选中的记录
	...
	private OutgoingDialForResourceWindow outgoingDialForResourceWindow;						// 呼叫弹出窗口
	...
	public OutgoingRecord2ResourceCreator(OutgoingDialForResourceWindow outgoingDialForResourceWindow) {
		this.setSpacing(true);
		this.outgoingDialForResourceWindow = outgoingDialForResourceWindow;
...
//		老版本
//		outgoingRecordStatuses = serviceRecordStatusService.getAllByDirection(loginUser.getDomain(), "outgoing");
	...
	private boolean saveChanges() {
		...
		boolean isSaveResourceSuccess = outgoingDialForResourceWindow.checkAndSaveCustomerResourceInfo();
		...
		try {
			...
			prprietaryCustomersTableFlip.refreshInCurrentPage();
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 */
	private void createOutgoingRecord(Date orderTime, String orderNote, MarketingProject currentWorkingProject) {
		// jrh 2013-07-22 统计录音文件名
		String dateStr = dateFormat.format(new Date());
		String recordFileName = dateStr+"-"+outgoingDialForResourceWindow.getAgiChannel().getUniqueId()+".wav";
	 ...
	 */
	private void createVipCustomerToProject(MarketingProject currentWorkingProject) {
	...
	public void setPrprietaryCustomersTableFlip(FlipOverTableComponent<CustomerResource> prprietaryCustomersTableFlip) {
