...
public class OutgoingDialForResourceWindow extends Window implements CloseListener {
	...
	private OutgoingRecord2ResourceCreator vipOutgoingRecordCreator;			// 外呼结果的创建界面
	...
	public OutgoingDialForResourceWindow() {
...
//		customerBaseInfoView.setCustomerInfoPanelHeight("165px");
		...
		// 创建客服记录
		vipOutgoingRecordCreator = new OutgoingRecord2ResourceCreator(this);
		vipOutgoingRecordCreator.setMargin(true);
		businessOperations.addTab(vipOutgoingRecordCreator, "创建服务记录", ResourceDataCsr.create_record_16_ico);
	...
	public void echoInformations(CustomerResource customerResource) {
		...
		// 第二步：回显个组件中的信息 vipOutgoingRecordCreator
		vipOutgoingRecordCreator.setCustomerResource(customerResource);				// 回显创建记录组件中客户的级别
	...
	public void windowClose(CloseEvent e) {
		vipOutgoingRecordCreator.clearComponentsValue();
		if(commodityOrderCreator != null) {
	...
	/**
	 * 为我的专有客户界面使用 ProprietaryCustomersTabView
	 * @param proprietaryCustomersTableFlip 存放客户信息的表格的翻页组件
	 */
	public void setProprietaryCustomersTableFlip(FlipOverTableComponent<CustomerResource> proprietaryCustomersTableFlip) {
		vipOutgoingRecordCreator.setPrprietaryCustomersTableFlip(proprietaryCustomersTableFlip);
	}
