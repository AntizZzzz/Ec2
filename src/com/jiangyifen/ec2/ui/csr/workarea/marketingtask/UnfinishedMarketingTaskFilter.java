...
public class UnfinishedMarketingTaskFilter extends VerticalLayout implements ClickListener, Content {
	...
	private FlipOverTableComponent<MarketingProjectTask> unfinishedTableFlip;		// unfinishedTaskTable的翻页组件
	
	public UnfinishedMarketingTaskFilter() {
	...
	public void buttonClick(ClickEvent event) {
		...
		if(source == searchButton && currentProjectId != 0) {
			...
			unfinishedTableFlip.setSearchSql(searchSql);
			unfinishedTableFlip.setCountSql(countSql);
			unfinishedTableFlip.refreshToFirstPage();
			
			// 在查询到结果后，默认选择 Table 的第一条记录
			BeanItemContainer<MarketingProjectTask> taskBeanItemContainer = unfinishedTableFlip.getEntityContainer();
	...
	private String  createCountSql() {
		...
		String inputName = StringUtils.trimToEmpty((String)customerName_tf.getValue());
		...
		String sql = " and mpt.user.id = "+loginUser.getId() + " and mpt.marketingProject.id = " + currentProjectId + " and mpt.isFinished = false " 
				+ specificStartOrderTimeSql + specificFinishOrderTimeSql + customerNameSql + customerPhoneSql + customerIdSql;
...
//		String phoneNumber = StringUtils.trimToEmpty((String) customerPhone.getValue());
...
//	}
	...
	public void setUnfinishedTableFlip(FlipOverTableComponent<MarketingProjectTask> unfinishedTableFlip) {
		this.unfinishedTableFlip = unfinishedTableFlip;
		csrTaskTable = unfinishedTableFlip.getTable();
	}
