...
public class TaskOutgoingRecordCreator extends VerticalLayout implements ClickListener, ValueChangeListener {
	// 日志工具
	...
	private OptionGroup noAnsweredResultOptions;		// 任务完成情况单选项(未接通)
...
//	private Table focusTaskTable;						// 当前正在操作的任务表格
	...
	private CustomerResource customerResource;			// 当前要操作的资源对象
	...
	private OutgoingDialForTaskWindow outgoingDialForTaskWindow;					// 呼叫弹出窗口
	...
	public TaskOutgoingRecordCreator(OutgoingDialForTaskWindow outgoingDialForTaskWindow) {
		this.setSpacing(true);
		this.outgoingDialForTaskWindow = outgoingDialForTaskWindow;
	...
	private void createMainComponents() {
		...
		noAnsweredResultOptions.addStyleName("boldcaption");	// 按三列排版
		...
		answeredResultOptions.addStyleName("boldcaption");		// 按三列排版
	...
	public void buttonClick(ClickEvent event) {
		...
		if(sourceButton == saveClose_button) {
			...
			if(success) { // 如果成功，移除窗口
				saveClose_button.getApplication().getMainWindow().removeWindow(TaskOutgoingRecordCreator.this.getWindow());
			}
	...
	private boolean saveChanges() {
		...
		boolean isSaveResourceSuccess = outgoingDialForTaskWindow.checkAndSaveCustomerResourceInfo();
		...
		try {
			...
			if(currentTask != null) {
				currentProject = currentTask.getMarketingProject();
			} else {
				logger.error("JRH - 20140606注 : 我的任务模块，无法获取到当前坐席拨打的是那一条任务，属于异常情况，注意检查。");
			}
			...
			// 创建一个新的外呼记录
			createOutgoingRecord(orderTime, orderNote, customerResource, currentProject);
			
			// 设置客户为 VIP 资源
			createVipCustomerToProject(customerResource, currentProject);
			
			// 更新客户级别（客户等级、客户经理）
			updateCustomerBaseInfo(customerResource);

			// 更新当前选中项任务的内容
			updateProjectTask(orderTime, orderNote);
			
			// TODO (为山西焦炭而实现) 自动完成任务迁移由土豆池1 转向 土豆池 2
			if(currentTask != null && currentProject != null && GlobalData.MAC_ADDRESS.equals(shanXiJiaoTanPcMac)) {
				// 自动完成任务迁移由土豆池1 转向 土豆池 2
	 ...
	 */
	private void updateProjectTask(Date orderTime, String orderNote) {
		if(currentTask != null) {
			currentTask.setLastStatus(serviceRecordStatus.toString());
			currentTask.setIsFinished(true);
			if(orderTime != null) {
				currentTask.setOrderTime(orderTime);
				currentTask.setOrderNote(orderNote);
			}
			if(serviceRecordStatus.getIsAnswered()) {
	 ...
	 * @param orderNote 预约内容
	 */
	private void createOutgoingRecord(Date orderTime, String orderNote, CustomerResource customerResource, MarketingProject marketingProject) {
		// jrh 2013-07-22 统计录音文件名
		String dateStr = dateFormat.format(new Date());
		String recordFileName = dateStr+"-"+outgoingDialForTaskWindow.getAgiChannel().getUniqueId()+".wav";
		...
		serviceRecord.setMarketingProject(marketingProject);
	 ...
	 */
	private void createVipCustomerToProject(CustomerResource customerResource, MarketingProject marketingProject) {
		...
		if(marketingProject != null) {
			...
			for(ProjectCustomer projectCustomer : projectCustomerService.getAllByProject(loginUser, marketingProject)) {
				MarketingProject project = projectCustomer.getAccountProject();
				if(project.getId() != null && project.getId().equals(marketingProject.getId())) {
					isVipInProject = true;
					break;
				}
			}
			...
			if(isVipInProject == false) {
				...
				projectCustomer.setAccountProject(marketingProject);
		...
		if(nextOrderProject != null) {
			...
			if(resourceBatch != null) {
				...
				if(existedInBatch == false) {
					...
					Long workflowTransferLogId = currentTask.getWorkflowTransferLogId();
					WorkflowTransferLog transferLog = null;
					
					// 第二步创建工作流迁移日志
					//---------------------  工作流迁移日志对应关联的资源的各种基础信息    ------------------//
					if(workflowTransferLogId != null) {
						transferLog = workflowTransferLogService.getWorkflowTransferLogById(workflowTransferLogId);
					} else {	// 正常情况下，这种情况不会出现，主要是为了适应以前已经指派出去的任务
					...
					if("workflow2".equals(workflow)) {
						if(workflowTransferLogId == null) {	// 新建日志则传递一销人员信息
						...
						if(workflowTransferLogId == null) {	// 新建日志则传递二销人员信息
