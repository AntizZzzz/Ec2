...
/**
 * 用于打完电话后，保存呼叫记录
 * 	供 CustomDialPopupWindow 使用
 */
@SuppressWarnings("serial")
public class CustomDialRecordInfoCreator extends VerticalLayout implements ValueChangeListener, ClickListener {
	// 日志工具
	...
	private CustomerServiceRecordStatus serviceRecordStatus;						// 当前被选中的‘客服记录状态’
	...
	private Object sourcePopWindow;													// 呼叫弹出窗口
	
	private BeanItemContainer<CustomerLevel> customerLevelContainer;							// 客户级别容器
	private BeanItemContainer<CustomerServiceRecordStatus> outgoingNoansweredStatusContainer;	// 外呼任务完成状态（代表未接通）
	...
	public CustomDialRecordInfoCreator(Object sourcePopWindow) {
		this.setSpacing(true);
		this.sourcePopWindow = sourcePopWindow;
	...
	private void createMainComponents() {
		...
		noAnsweredResultOptions.addStyleName("boldcaption");
	...
	private boolean saveChanges() {
		if(sourcePopWindow != null) {
			Class sourceWindowClass = null;
			try{
				// 如果客户基础信息处于编辑状态，则先保存客户基础信息，然后再创建后续记录
				sourceWindowClass = sourcePopWindow.getClass();
				Method sourceWindowMethod = sourceWindowClass.getMethod("checkAndSaveCustomerResourceInfo");
				boolean isSaveResourceSuccess = (Boolean) sourceWindowMethod.invoke(sourcePopWindow);
				...
				logger.warn("Class "+sourceWindowClass.getName()+" 调用 checkAndSaveCustomerResourceInfo() 方法失败!");
				this.getApplication().getMainWindow().showNotification("客户基础信息填写有误，保存失败！", Notification.TYPE_WARNING_MESSAGE);
				return false;
			}
		} else {
			logger.info("创建客服记录的组件找不到弹屏窗口,创建弹屏界面的组件没有传递 sourcePopWindow");
			this.getApplication().getMainWindow().showNotification("创建客服记录的组件找不到弹屏窗口，保存失败！", Notification.TYPE_WARNING_MESSAGE);
			return false;
		}
		...
		try {
			...
			// 创建一个新的外呼记录
			createOutgoingRecord(orderTime, orderNote, currentProject);
			
			// 设置客户为 VIP 资源
			createVipCustomerToProject(currentProject);
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private void createOutgoingRecord(Date orderTime, String orderNote, MarketingProject project) {
		// jrh 2013-07-22 统计录音文件名
		String dateStr = dateFormat.format(new Date());
		String recordFileName = "";
//		String recordFileName = dateStr+"-"+customDialPopupWindow.getAgiChannel().getUniqueId()+".wav";
		if(sourcePopWindow != null) {
			Class sourceWindowClass = null;
			try{
				// 如果客户基础信息处于编辑状态，则先保存客户基础信息，然后再创建后续记录
				sourceWindowClass = sourcePopWindow.getClass();
				Method sourceWindowMethod = sourceWindowClass.getMethod("getAgiChannel");
				AgiChannel agiChannel = (AgiChannel) sourceWindowMethod.invoke(sourcePopWindow);
				if(agiChannel != null) {
					recordFileName = dateStr+"-"+agiChannel.getUniqueId()+".wav";
				}
			} catch (Exception e) {
				logger.warn("Class "+sourceWindowClass.getName()+" 调用 getAgiChannel() 方法失败!");
			}
		} else {
			logger.info("创建客服记录的组件找不到弹屏窗口,创建弹屏界面的组件没有传递 sourcePopWindow");
			this.getApplication().getMainWindow().showNotification("创建客服记录的组件找不到弹屏窗口，保存失败！", Notification.TYPE_WARNING_MESSAGE);
	 ...
	 */
	private void createVipCustomerToProject(MarketingProject project) {
		...
		if(project != null) {
			...
			for(ProjectCustomer projectCustomer : projectCustomerService.getAllByProject(loginUser, project)) {
				...
				if(marketingProject.getId() != null && marketingProject.getId().equals(project.getId())) {
					isVipInProject = true;
					break;
				}
			}
