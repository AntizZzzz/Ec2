...
//		this.getWindow().addWindow(editNoticeWindow);
	...
	private void executeSearch() {
		...
		sqlSelect = "select s from MailHistory as s where s.subject like '%"+keyWordStr+"%' and s.content like '%"+keyWordStr+"%' and s.loginUser.id = " + loginUser.getId() + " order by s.id desc";
		sqlCount = "select count(s.id) from MailHistory as s where s.subject like '%"+keyWordStr+"%' and s.content like '%"+keyWordStr+"%' and s.loginUser.id = " + loginUser.getId();
