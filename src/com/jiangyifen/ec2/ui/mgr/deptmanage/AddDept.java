...
public class AddDept extends Window implements Button.ClickListener {
 ...
 */
	 ...
	 * @param project
	 */
	public AddDept(DeptManagement deptManagement) {
		this.center();
		this.setModal(true);
	...
	private boolean executeSave() {
		try {
			...
			deptManagement.updateTable(true);
			deptManagement.getTable().setValue(null);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private class MyFieldFactory extends DefaultFieldFactory{
		private List<Department> toRemoveDepts;
		@Override
		public Field createField(Item item, Object propertyId,
			...
			if("name".equals(propertyId)){
				...
				deptContainer.addAll(deptService.getAll(SpringContextHolder.getDomain()));
				//排除子部门
				excludeSubDepartments(deptContainer);
		...
		//排除自己部门和所有子部门
		private void excludeSubDepartments(BeanItemContainer<Department> deptContainer) {
			toRemoveDepts=new ArrayList<Department>();
			toRemoveDepts.size();
			//TODO chb 移除子部门及自身 
		}
	}
