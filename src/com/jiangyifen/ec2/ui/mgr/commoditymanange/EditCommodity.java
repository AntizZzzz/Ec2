...
public class EditCommodity extends Window implements Button.ClickListener {
/**
 * 主要组件输出	
 */
	//Form输出
...
//	private Domain domain;				// 当前登陆商品所属域
	 ...
	 * @param project
	 */
	public EditCommodity(CommodityManagement commodityManagement) {
		this.center();
		this.setModal(true);
...
//		domain = SpringContextHolder.getDomain();
	...
	private boolean executeSave() {
		try {
			...
			if(UiUtil.getComponentValue(commodityPrice).equals("")){
			...
			// 商品库存
			String stockQtyStr = StringUtils.trimToEmpty((String)(stockQty_tf.getValue().toString()));
	...
	public void attach() {
		super.attach();
		//取出商品
		commodity=(Commodity)commodityManagement.getTable().getValue();
		//设置显示的内容
		commodityName.setValue(commodity.getCommodityName());
		commodityPrice.setValue(commodity.getCommodityPrice());
		stockQty_tf.setValue(commodity.getStockQty());
		commodityStatusComboBox.setValue(commodity.getCommodityStatus());
		description.setValue(commodity.getDescription());
	}
