...
/**
 * 
 * @Description 描述：用于编辑外线与IVR 的关联界面
 * 
 * @author  jrh
...
public class EditOtilWindow extends Window implements Button.ClickListener, ValueChangeListener {
	...
	public EditOtilWindow(OutlineToIvrLinkManageView otilManageView) {
		this.setCaption("编辑 (按外线配置呼入IVR)");
		this.center();
		this.setModal(true);
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if(source == dateType_og) {
			...
			if("forever".equals(type)) {
				...
				effectDate_pdf.setValue(otil.getEffectDate());
				expireDate_pdf.setValue(otil.getExpireDate());
	...
	private void excuteSave() {
		...
		// 创建新对象
		otil.setIvrMenuId(ivrMenu.getId());
		otil.setEffectDate(effectDate);
		otil.setExpireDate(expireDate);
		otil.setStartHour(startHour);
		otil.setStartMin(startMin);
		otil.setStopHour(stopHour);
		otil.setStopMin(stopMin);
		otil.setIsMonEffect(dayOfWeeks.contains(DayOfWeek.mon));
		otil.setIsTueEffect(dayOfWeeks.contains(DayOfWeek.tue));
		otil.setIsWedEffect(dayOfWeeks.contains(DayOfWeek.wen));
		otil.setIsThuEffect(dayOfWeeks.contains(DayOfWeek.thu));
		otil.setIsFriEffect(dayOfWeeks.contains(DayOfWeek.fri));
		otil.setIsSatEffect(dayOfWeeks.contains(DayOfWeek.sat));
		otil.setIsSunEffect(dayOfWeeks.contains(DayOfWeek.sun));
		
		outlineToIvrLinkService.update(otil);	// 执行更新

		// 执行成功后，更新内存
		ArrayList<OutlineToIvrLink> otilArrLs = ShareData.outlineIdToIvrLinkMap.get(outline.getId());
		if(otilArrLs == null) {	// 正常情况下不会出现
			...
			for(OutlineToIvrLink link : otilArrLs) {
				if(link.getId().equals(otil.getId())) {
					index = otilArrLs.indexOf(link);
					break;
				}
			}
			otilArrLs.set(index, otil);
		}
	 ...
	 */
	public void updateUiDataSouce(SipConfig outline, OutlineToIvrLink otil) {
		...
		ivrMenuContainer.addAll(ivrMenus);

		// 回显信息
		for(IVRMenu menu : ivrMenus) {
			if(menu.getId().equals(otil.getIvrMenuId())) {
				ivrMenus_cb.setValue(menu);
				break;
			}
		}
		
		dateType_og.setValue("custom");
		effectDate_hlo.setVisible(true);
		effectDate_pdf.setValue(otil.getEffectDate());
		expireDate_pdf.setValue(otil.getExpireDate());
		startHour_cb.setValue(otil.getStartHour());
		startMin_cb.setValue(otil.getStartMin());
		stopHour_cb.setValue(otil.getStopHour());
		stopMin_cb.setValue(otil.getStopMin());
		dayOfWeekType_og.setValue("custom");
		
		daysOfWeek_og.setReadOnly(false);
		Set<DayOfWeek> days = new HashSet<DayOfWeek>();
		if(otil.getIsMonEffect()) {
			days.add(DayOfWeek.mon);
		}
		if(otil.getIsTueEffect()) {
			days.add(DayOfWeek.tue);
		}
		if(otil.getIsWedEffect()) {
			days.add(DayOfWeek.wen);
		}
		if(otil.getIsThuEffect()) {
			days.add(DayOfWeek.thu);
		}
		if(otil.getIsFriEffect()) {
			days.add(DayOfWeek.fri);
		}
		if(otil.getIsSatEffect()) {
			days.add(DayOfWeek.sat);
		}
		if(otil.getIsSunEffect()) {
			days.add(DayOfWeek.sun);
		}
		daysOfWeek_og.setValue(days);
	}
