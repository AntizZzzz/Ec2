...
/**
 * 
 * @Description 描述：批量修改外线配置项
 * 
 * @author  jrh
 * @date    2014年1月22日 下午1:16:59
 * @version v1.0.0
 */
@SuppressWarnings("serial")
public class ConfMultiOutlineWindow extends Window implements ClickListener {
	// 日志工具
	private final Logger logger = LoggerFactory.getLogger(this.getClass());

	// 要修改的列的标题名称
	private final String[] CAPTION_NAMES = new String[] {"name", "secret", "context", "type", "host", "qualify", 
			"canreinvite", "disallow", "allow", "nat", "port", "cancallforward", "call_limit", "belong", "ispopupWin"};

	private final Object[] CAPTION_NAMES_CAPTION = new Object[] { "外线名称", "密码","呼叫路由","注册类型","SIP服务器","心跳认证",
			"二次拨号", "不允许编码", "允许编码", "内网穿透","注册端口","呼叫流转","并发限制","归属地","是否弹屏"};
	
	// 上述列对应的组件的默认值
	private final Object[] DEFAULT_VALUES = new Object[] {"", "", "incoming", "friend", 
			"", "yes", "yes", "all", "g729", "yes", "5060", "yes", "10", "", true};

	// 上述列对应的组件默认的可用状态
	private Boolean[] COL2_ENABLES = new Boolean[]{true, false, false, false, false, false, 
	...
	private TextField calllimit_tf;
	private ComboBox belong_cb;
	private ComboBox ispopupWin_cb;
	...
	private Button calllimit_bt; 
	private Button belong_bt; 
	private Button ispopupWin_bt; 
	...
	public ConfMultiOutlineWindow(OutlineManagement outlineManagement) {
		this.center();
		this.setModal(true);
		this.setResizable(false);
		this.setWidth("490px");
		this.setHeight("500px");
		this.setCaption("批量配置外线");
	...
	public void attach() {
		...
		LinkedHashMap<Long,SipConfig> needEditSips = outlineManagement.getNeedDeleteSips();
	...
	private void createCol2Componets() {
		...
		calllimit_tf = createTextField(++row, "\\d{1,4}", 4);
		calllimit_tf.setValue(DEFAULT_VALUES[row]);
		belong_cb = createComboBox(++row, new String[] {});
		for(MobileAreacode areacode : ShareData.areacodeList){
			belong_cb.addItem(areacode.getAreaCode());
			belong_cb.setItemCaption(areacode.getAreaCode(),areacode.getMobileArea()+"("+areacode.getAreaCode()+")");
		}
		ispopupWin_cb = createComboBox(++row, new Boolean[] {true, false});
		ispopupWin_cb.setItemCaption(true, "呼叫-弹屏");
		ispopupWin_cb.setItemCaption(false, "呼叫-不弹屏");
	}
	...
	private void createCol3Componets() {
		...
		calllimit_bt = createOptButton(++row);
		belong_bt = createOptButton(++row);
		ispopupWin_bt = createOptButton(++row);
	}
	...
	public void buttonClick(ClickEvent event) {
		...
		} else if(source == calllimit_bt) {
			updateTextFieldStatus(calllimit_bt, calllimit_tf);
		} else if(source == belong_bt) {
			updateComboBoxStatus(belong_bt, belong_cb);
		} else if(source == ispopupWin_bt) {
			updateComboBoxStatus(ispopupWin_bt, ispopupWin_cb);
		} 
	}
	...
	private boolean executeSave() {
		try {
			...
			if(!secret_tf.isEnabled() && !context_tf.isEnabled() && !type_cb.isEnabled() 
					...
					&& !disallow_tf.isEnabled() && !allow_tf.isEnabled() && !nat_cb.isEnabled() 
					&& !port_tf.isEnabled() && !cancallforward_cb.isEnabled() && !calllimit_tf.isEnabled()
					&& !belong_cb.isEnabled() && !ispopupWin_cb.isEnabled()) {
			...
			String calllimit = StringUtils.trimToEmpty((String) calllimit_tf.getValue());
			String belong = StringUtils.trimToEmpty((String) belong_cb.getValue());
			Boolean ispopupWin = (Boolean) ispopupWin_cb.getValue();
			...
			if(secret_tf.isEnabled() && (!secret_tf.isValid() || "".equals(secret))) {
				...
				return false;
			} else if(calllimit_tf.isEnabled() && (!calllimit_tf.isValid() || "".equals(calllimit))) {
				this.getApplication().getMainWindow().showNotification("call-limit 只能由长度不大于4位的数字组成，且不能为空", Notification.TYPE_WARNING_MESSAGE);
				return false;
			} else if(belong_cb.isEnabled() && (!belong_cb.isValid() || "".equals(belong))) {
				this.getApplication().getMainWindow().showNotification("belong 不能为空", Notification.TYPE_WARNING_MESSAGE);
				return false;
			}
			...
			for(SipConfig sip : needEditSips.values()) {
				...
				if(!"".equals(calllimit)) {
					sip.setCall_limit(Integer.parseInt(calllimit));
				}
				if(!"".equals(belong)) {
					sip.setBelong(belong);
				}
				if(ispopupWin != null) {
					sip.setIspopupWin(ispopupWin);
				}
				sipConfigService.update(sip);
			}
			
			//刷新sipManagement的Table
			outlineManagement.clearNeedDeleteSips(); 
			outlineManagement.updateTable(false); 
			outlineManagement.getTable().setValue(null); 
			this.getApplication().getMainWindow().showNotification("保存外线成功！");
		} catch (Exception e) {
