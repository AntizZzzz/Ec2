...
public class FlipOverTableComponentUseNativeSql<T> extends HorizontalLayout implements
...
//	private TextField inputField;
	...
	private FlipSupportNativeSqlService<T> flipSupportService;
	 ...
	 */
	public FlipOverTableComponentUseNativeSql(Class<T> type, FlipSupportNativeSqlService<T> flipSupportService, Table table,
	...
	private void loadPage(int pageNum) {
		...
		List<T> entitys = flipSupportService.loadPageEntitiesByNativeSql(
...
//		}
		...
		showNo.setValue(pagination.getCurrentPage() + "/"
				+ pagination.getTotalPage() + "页" + "--总记录数："
	...
	private void rebuildPagination() {
		pagination.setTotalRecord(flipSupportService.getEntityCountByNativeSql(countSql));
	}
...
//		this.addComponent(inputField);
	...
	/**
	 * jrh 设置翻页组件的每页记录条数并且刷新界面，默认为15条
	 */
	public void setPageLengthAndRefresh(int pageLength) {
		pagination.setPageRecords(pageLength);
		refreshToFirstPage();
	}
