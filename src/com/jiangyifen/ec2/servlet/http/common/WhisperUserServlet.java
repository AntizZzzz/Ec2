...
/**
 * @Description 描述：调用该接口，用于跟指定坐席进行密语
 * 
 * 	坐席必须同时具备以下条件，方可进行密语：
 * 
 * 		假设：管理员使用分机800008要正在使用分机800001进行通话的坐席1001执行密语
 * 
 * 		1、管理员用于密语的分机800008，暂时没有任何坐席与之绑定 
 * 		1、坐席1001跟存在（即要求用户名参数要正确）
 * 		2、坐席1001已经与分机800001建立了绑定关系（即相当于登录了EC2系统）
 * 		3、坐席使用的分机800001存在对应的通道，即存在单通，或者已经接通的电话
 * 
 * 请求路径：http://{ec2_server_ip}:{ec2_server_port}/ec2/http/common/whisperUser?accessId=xxx&accessKey=xxx&destUsername=1001&mgrExten=800008
 *
 * eg.
 * 	http://192.168.2.160:8080/ec2/http/common/whisperUser?accessId=xxx&accessKey=xxx&destUsername=1001&mgrExten=800008
 * 
 * @author  JRH
 * @date    2014年8月12日 上午11:00:01
 */
@SuppressWarnings("serial")
public class WhisperUserServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if("".equals(destUsername) || "".equals(mgrExten)) {
				...
				logger.warn("JRH - IFACE 密语失败，原因：请求参数中目标用户名destUsername 或 分机号exten 值为空！");
			...
			if(userId != null) {	// 检查分机被占用情况
				...
				logger.warn("JRH - IFACE 密语失败，原因：当前分机已被坐席占用！");
				operateResponse(response, commonRespBo);
				return;
			}
			...
			if (destUserLs == null || destUserLs.size() == 0) {
				...
				logger.warn("JRH - IFACE 密语失败，原因：请求参数中用户名有误，租户编号"+domainId+"下，目标用户"+destUsername+"不存在！");
				operateResponse(response, commonRespBo);
				return;
			}
			...
			if(csrExten == null) {	// 检查在线情况
				...
				logger.warn("JRH - IFACE 密语失败，原因：目标用户"+destUsername+"尚未绑定分机！");
				operateResponse(response, commonRespBo);
				return;
			}
			...
			if(csrChannels != null && csrChannels.size() == 0){
				...
				logger.warn("JRH - IFACE 密语失败，原因：目标用户"+destUsername+"当前没有通话！");
				operateResponse(response, commonRespBo);
				return;
			}
			...
			logger.error("JRH - IFACE 密语, 出现异常！"+e.getMessage(), e);
		}
	}
