...
/**
 * 测试 -- 在推送 CDR 话单的同时, 查询 /dev/shm 文件下是否存在这个推送的文件, 然后查询指定的文件夹下是否存在这个推送文件
 *
 * @author jinht
 *
 * @date 2015-7-15 下午4:47:16 
 *
 */
@Entity
@Table(name = "cdr")
public class Cdr_jinht_bak {
	@Id
...
//		return null;
	...
	public String getRealUrl(String baseUrl) {
		...
		// jinht 判断临时目录下是否存在该文件
		String filePath1 = Config.props.getProperty(Config.REC_LOCAL_MEM_PATH) + "/" + dateStr + "-" + uniqueId + ".wav";
		String realUrl1 = baseUrl + dateStr + "/" + dateStr + "-" + uniqueId + ".wav";
		File file1 = new File(filePath1);
		if(file1.exists()) {
			return realUrl1;
		}
		...
		String realUrl2 = baseUrl+dateStr+"/"+dateStr+"-"+uniqueId+".wav";
